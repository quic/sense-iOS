...
func removeAlpha(srcPixelBuffer: CVPixelBuffer, width: Int, height: Int) -> CVPixelBuffer? {
    
    CVPixelBufferLockBaseAddress(srcPixelBuffer, CVPixelBufferLockFlags(rawValue: 0))
    defer { CVPixelBufferUnlockBaseAddress(srcPixelBuffer, CVPixelBufferLockFlags(rawValue: 0)) }

    guard let destData = malloc(width*height*3) else {
        print("Out of memory")
        return nil
    }
    let srcData = CVPixelBufferGetBaseAddress(srcPixelBuffer)
    var srcBuffer = vImage_Buffer(data: srcData, height: vImagePixelCount(height), width: vImagePixelCount(width), rowBytes: 4*width)
    
    var dstBuffer = vImage_Buffer(data: destData, height: vImagePixelCount(height), width: vImagePixelCount(width), rowBytes: 3*width)
    
    vImageConvert_ARGB8888toRGB888(&srcBuffer, &dstBuffer, vImage_Flags(0))
    
    let destBytesPerRow = 3*width
    ...
    let status = CVPixelBufferCreateWithBytes(nil, width, height,
                                              kCVPixelFormatType_24RGB, destData,
                                              destBytesPerRow, releaseCallback,
                                              nil, nil, &dstPixelBuffer)
